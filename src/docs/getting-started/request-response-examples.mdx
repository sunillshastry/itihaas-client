...
As mentioned, the Itihaas API adheres to RESTful principles and architecture, returning data in **JSON** format, and uses the standard HTTP status codes for success, not-found and error states; a helpful guide concerning HTTP status codes can be found [here](https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Status 'MDN Docs for HTTP Status Codes'). Additionally, Itihaas also follows the `lite-api` JSON specification; <span className="underline">[`lite-api`](https://github.com/sunillshastry/lite-api 'lite-api Specification')</span> is a simple JSON API-specification format used for web applications to build APIs in JSON format for a consistent and standardized way. Although we do not wish to delve deeper into `lite-api`, here is a basic (simplified) structure of how a JSON API would look with the `lite-api` specification:
...
> A full guide to `lite-api` specification documentation can be viewed [here](https://github.com/sunillshastry/lite-api 'Full lite-api specification').
